...
<head>
    ...
    <title>Live Monitoring Map - SIDAJU Jakarta Utara</title>
    ...
    <div class="absolute top-6 left-0 right-0 z-10 px-4 pointer-events-none flex flex-col md:flex-row justify-between items-start gap-4">
        ...
        <div class="pointer-events-auto flex flex-col gap-3 ml-2 md:ml-6">
            ...
            <div class="glass-panel px-5 py-4 rounded-2xl shadow-xl flex items-center gap-4">
                ...
                <div>
                    <h1 class="font-display font-bold text-lg text-white leading-none tracking-wide">Live Control Room</h1>
                    <p class="text-sky-400 text-xs font-semibold uppercase tracking-widest mt-1">SDA Jakarta Utara</p>
                </div>
            </div>
        </div>
    ...
    <div class="absolute bottom-8 left-0 right-0 z-10 px-4 pointer-events-none flex justify-center">
        <div class="pointer-events-auto glass-panel p-4 md:p-5 rounded-2xl shadow-2xl flex flex-col md:flex-row items-center gap-6 md:gap-8 max-w-full overflow-x-auto">
            ...
            <div class="flex items-center gap-3 shrink-0">
                ...
                <div>
                    <p class="text-white text-sm font-bold leading-none">Pintu Air</p>
                    <p class="text-slate-400 text-[10px] font-medium uppercase tracking-wider mt-1">Aktif</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 shrink-0">
                <div class="w-8 h-8 rounded-full flex items-center justify-center shadow-lg" style="background-color: #f97316;">
                    <i class="fa-solid fa-fan text-white text-xs"></i>
                </div>
                <div>
                    <p class="text-white text-sm font-bold leading-none">Pompa Station</p>
                    <p class="text-slate-400 text-[10px] font-medium uppercase tracking-wider mt-1">Aktif</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 shrink-0">
                ...
                <div>
                    <p class="text-white text-sm font-bold leading-none">Pompa Mobile</p>
                    <p class="text-slate-400 text-[10px] font-medium uppercase tracking-wider mt-1">Aktif</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 shrink-0">
                <div class="w-8 h-8 rounded-full flex items-center justify-center shadow-lg" style="background-color: #ec4899;">
                    <i class="fa-solid fa-layer-group text-white text-xs"></i>
                </div>
                <div>
                    <p class="text-white text-sm font-bold leading-none">Sub Polder</p>
                    <p class="text-slate-400 text-[10px] font-medium uppercase tracking-wider mt-1">Aktif</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 shrink-0 opacity-80">
                <div class="w-8 h-8 rounded-full flex items-center justify-center shadow-lg border-2 border-red-500 bg-slate-800">
                    <i class="fa-solid fa-triangle-exclamation text-red-500 text-xs"></i>
                </div>
                <div>
                    <p class="text-red-400 text-sm font-bold leading-none">Perbaikan</p>
                    <p class="text-slate-500 text-[10px] font-medium uppercase tracking-wider mt-1">Maintenance</p>
                </div>
            </div>
            ...
            if (locations && locations.length > 0) {
                ...
                locations.forEach(loc => {
                    if (loc.lat && loc.lng && !isNaN(loc.lat) && !isNaN(loc.lng)) {
                        ...
                        let icon = 'fa-circle';
                        if (loc.type === 'Pintu Air') icon = 'fa-water';
                        if (loc.type === 'Pompa') icon = 'fa-fan';
                        if (loc.type === 'Pompa Mobile') icon = 'fa-truck';
                        if (loc.type === 'Sub Polder') icon = 'fa-layer-group';
                        
                        if (loc.status === 'Perbaikan') icon = 'fa-triangle-exclamation';
                            ...
                            <div class="p-5 max-w-sm w-72">
                                ...
                                <div class="space-y-3 mb-5">
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs text-slate-400">Status Operasional</span>
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold ${loc.status === 'Aktif' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'} border border-current">
                                            <span class="w-1.5 h-1.5 rounded-full mr-1.5 ${loc.status === 'Aktif' ? 'bg-emerald-400 animate-pulse' : 'bg-red-400'}"></span>
                                            ${loc.status}
                                        </span>
                                    </div>
                                    <div class="flex items-center justify-between">
