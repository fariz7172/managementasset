...
@section('title', 'Edit Data Sensus')
@section('page_title', 'Edit Data Sensus')
...
<div class="glass-card animate-fade-in delay-1" style="max-width: 100%;">
    <div class="card-header" style="margin-bottom: 24px; display: flex; justify-content: space-between; align-items: center;">
        <div>
            <h2 class="card-title" style="margin: 0; font-size: 1.25rem;">Edit Pendataan Sensus</h2>
            <p style="font-size: 0.875rem; color: var(--color-text-muted); margin: 4px 0 0 0;">Ubah data sensus yang sudah tersimpan.</p>
        </div>
    ...
    <form action="{{ route('sensus.update', $sensus->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="sensus-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 24px;">
                ...
                <div style="margin-bottom: 16px;">
                    ...
                    <select name="status_pelaksanaan" class="form-control" style="appearance: auto;">
                        ...
                        <option value="Belum Inventarisasi" {{ old('status_pelaksanaan', $sensus->status_pelaksanaan) == 'Belum Inventarisasi' ? 'selected' : '' }}>Belum Inventarisasi</option>
                        <option value="Menunggu Inventarisasi Mobile" {{ old('status_pelaksanaan', $sensus->status_pelaksanaan) == 'Menunggu Inventarisasi Mobile' ? 'selected' : '' }}>Menunggu Inventarisasi Mobile</option>
                        <option value="Disetujui P3B" {{ old('status_pelaksanaan', $sensus->status_pelaksanaan) == 'Disetujui P3B' ? 'selected' : '' }}>Disetujui P3B</option>
                    </select>
                </div>
                
                <div style="margin-bottom: 16px;">
                    <label class="form-label">Kode Barang</label>
                    <input type="text" name="kode_barang" class="form-control" value="{{ old('kode_barang', $sensus->kode_barang) }}">
                </div>
                
                <div style="margin-bottom: 16px;">
                    <label class="form-label">Nama Barang</label>
                    <input type="text" name="nama_barang" class="form-control" value="{{ old('nama_barang', $sensus->nama_barang) }}">
                </div>
                
                <div style="margin-bottom: 16px;">
                    <label class="form-label">Nomor Register</label>
                    <input type="text" name="nomor_register" class="form-control" value="{{ old('nomor_register', $sensus->nomor_register) }}">
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Tanggal Perolehan</label>
                    <input type="date" name="tanggal_perolehan" class="form-control" value="{{ old('tanggal_perolehan', $sensus->tanggal_perolehan ? \Carbon\Carbon::parse($sensus->tanggal_perolehan)->format('Y-m-d') : '') }}">
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Harga (Rp)</label>
                    <input type="number" step="0.01" name="harga" class="form-control" value="{{ old('harga', $sensus->harga) }}">
                </div>
                
                <div style="margin-bottom: 16px;">
                    <label class="form-label">Objek</label>
                    <input type="text" name="objek" class="form-control" value="{{ old('objek', $sensus->objek) }}">
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Nama Objek</label>
                    <input type="text" name="nama_objek" class="form-control" value="{{ old('nama_objek', $sensus->nama_objek) }}">
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Sub Rincian Objek</label>
                    <input type="text" name="sub_rincian_objek" class="form-control" value="{{ old('sub_rincian_objek', $sensus->sub_rincian_objek) }}">
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Nama Sub Rincian Objek</label>
                    <input type="text" name="nama_sub_rincian_objek" class="form-control" value="{{ old('nama_sub_rincian_objek', $sensus->nama_sub_rincian_objek) }}">
                </div>
            </div>
            ...
            <div style="background: var(--color-bg); padding: 20px; border-radius: 12px; border: 1px solid var(--border-color); box-shadow: var(--shadow-sm);">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                    <div>
                        ...
                        <input type="number" step="0.01" name="panjang" class="form-control" value="{{ old('panjang', $sensus->panjang) }}">
                    </div>
                    <div>
                        <label class="form-label">Lebar</label>
                        <input type="number" step="0.01" name="lebar" class="form-control" value="{{ old('lebar', $sensus->lebar) }}">
                    </div>
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Ukuran</label>
                    <input type="text" name="ukuran" class="form-control" value="{{ old('ukuran', $sensus->ukuran) }}">
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Satuan</label>
                    <input type="text" name="satuan" class="form-control" value="{{ old('satuan', $sensus->satuan) }}">
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Penggunaan</label>
                    <input type="text" name="penggunaan" class="form-control" value="{{ old('penggunaan', $sensus->penggunaan) }}">
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Keterangan Masalah</label>
                    <textarea name="ket_masalah" class="form-control" rows="3">{{ old('ket_masalah', $sensus->ket_masalah) }}</textarea>
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Pengembang</label>
                    <input type="text" name="pengembang" class="form-control" value="{{ old('pengembang', $sensus->pengembang) }}">
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Jenis Objek KIB D</label>
                    <input type="text" name="jenis_objek_kib_d" class="form-control" value="{{ old('jenis_objek_kib_d', $sensus->jenis_objek_kib_d) }}">
                </div>
            </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 24px;">
                ...
                <div style="margin-bottom: 16px;">
                    ...
                    <input type="text" name="nama_jalan_alamat" class="form-control" value="{{ old('nama_jalan_alamat', $sensus->nama_jalan_alamat) }}">
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                    <div>
                        <label class="form-label">No. Jalan</label>
                        <input type="text" name="nomor_jalan" class="form-control" value="{{ old('nomor_jalan', $sensus->nomor_jalan) }}">
                    </div>
                    <div>
                        <label class="form-label">RT</label>
                        <input type="text" name="rt" class="form-control" value="{{ old('rt', $sensus->rt) }}">
                    </div>
                    <div>
                        <label class="form-label">RW</label>
                        <input type="text" name="rw" class="form-control" value="{{ old('rw', $sensus->rw) }}">
                    </div>
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Kode Kelurahan</label>
                    <input type="text" name="kode_kelurahan" class="form-control" value="{{ old('kode_kelurahan', $sensus->kode_kelurahan) }}">
                </div>
                <div style="margin-bottom: 16px;">
                    <label class="form-label">Kelurahan</label>
                    <input type="text" name="kelurahan" class="form-control" value="{{ old('kelurahan', $sensus->kelurahan) }}">
                </div>
                <div style="margin-bottom: 16px;">
                    <label class="form-label">Kecamatan</label>
                    <input type="text" name="kecamatan" class="form-control" value="{{ old('kecamatan', $sensus->kecamatan) }}">
                </div>
                ...
                <div style="padding: 16px; background: rgba(227, 116, 52, 0.05); border-radius: 8px; border: 1px dashed rgba(227, 116, 52, 0.3); margin-bottom: 16px;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                        <div>
                            ...
                            <input type="text" name="latitude" id="edit_latitude" class="form-control" value="{{ old('latitude', $sensus->latitude) }}">
                        </div>
                        <div>
                            ...
                            <input type="text" name="longitude" id="edit_longitude" class="form-control" value="{{ old('longitude', $sensus->longitude) }}">
                        </div>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 16px;">
                    ...
                    <textarea name="alamat_foto_pendataan" class="form-control" rows="2">{{ old('alamat_foto_pendataan', $sensus->alamat_foto_pendataan) }}</textarea>
                </div>
                ...
                <div style="margin-bottom: 16px;">
                    ...
                    @if($sensus->url_foto_pendataan && is_array($sensus->url_foto_pendataan))
                        ...
                        <small style="color: var(--color-teal); display: block; margin-bottom: 8px;"><i class="fa-solid fa-info-circle"></i> Foto yang baru diunggah akan **ditambahkan** ke dalam daftar foto di atas (foto lama tidak dihapus).</small>
                    @endif
                    <input type="file" name="url_foto_pendataan[]" class="form-control" accept="image/*" multiple>
                    <small style="color: var(--color-text-muted); display: block; margin-top: 4px;">Anda dapat memilih lebih dari satu foto. Foto akan otomatis dikonversi ke WebP.</small>
                </div>
                ...
                <div style="margin-bottom: 16px;">
                    ...
                    <input type="date" name="dibuat_diperbaharui_pada" class="form-control" value="{{ old('dibuat_diperbaharui_pada', $sensus->dibuat_diperbaharui_pada ? \Carbon\Carbon::parse($sensus->dibuat_diperbaharui_pada)->format('Y-m-d') : '') }}">
                </div>

                <div style="margin-bottom: 16px;">
                    <label class="form-label">Dibuat/Diperbarui Oleh</label>
                    <input type="text" name="dibuat_diperbaharui_oleh" class="form-control" value="{{ old('dibuat_diperbaharui_oleh', $sensus->dibuat_diperbaharui_oleh) }}">
                </div>
            </div>
                </div>
        ...
        <div style="margin-top: 32px; display: flex; justify-content: flex-end; gap: 12px; border-top: 1px solid var(--border-color); padding-top: 24px;">
            ...
            <button type="submit" class="btn btn-primary" style="padding: 10px 24px; font-size: 1rem;">
                <i class="fa-solid fa-save"></i> Simpan Perubahan Sensus
            </button>
        </div>
    </form>
</div>
