...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Monitoring Map</h1>
            <p class="text-slate-500 text-sm mt-1">Pemantauan lokasi seluruh aset air secara *real-time*.</p>
        </div>
    </div>
    ...
    <div class="bg-white p-2 rounded-2xl border border-slate-200 shadow-sm relative">
        ...
        <div class="map-legend">
            ...
            <div class="legend-item">
                ...
                <span>Pompa Mobile (Aktif)</span>
            </div>
            <div class="legend-item">
                <div class="legend-color" style="background-color: #ec4899;"></div>
                <span>Sub Polder (Aktif)</span>
            </div>
            <hr class="my-2 border-slate-200">
            <div class="legend-item">
                <div class="legend-color" style="background-color: #ef4444; border: 2px solid white; box-shadow: 0 0 0 1px #ef4444;"></div>
                <span class="text-red-600 font-bold">Aset Perbaikan / Rusak</span>
            </div>
        </div>
    </div>
</div>
        ...
        if (locations && locations.length > 0) {
            ...
            locations.forEach(loc => {
                if (loc.lat && loc.lng && !isNaN(loc.lat) && !isNaN(loc.lng)) {
                    // Normalize coordinates if necessary (some DB entries might be flipped)
                    ...
                    const popupContent = `
                        <div class="p-3 max-w-xs">
                            <div class="text-xs font-bold uppercase tracking-wider mb-1" style="color: ${loc.color}">${loc.type}</div>
                            <h3 class="font-bold text-slate-900 mb-2">${loc.name}</h3>
                            <div class="flex items-center gap-2 mb-1">
                                <span class="text-xs text-slate-500 font-medium w-16">Status:</span>
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold ${loc.status === 'Aktif' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}">${loc.status}</span>
                            </div>
                            <div class="flex items-center gap-2 mb-3">
                                <span class="text-xs text-slate-500 font-medium w-16">Koor:</span>
                                <span class="text-xs font-mono text-slate-700">${lat.toFixed(4)}, ${lng.toFixed(4)}</span>
                            </div>
