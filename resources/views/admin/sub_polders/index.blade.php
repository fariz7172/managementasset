...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            <h1 class="text-3xl font-display font-bold text-slate-900">Data Sub Polder</h1>
            <p class="text-slate-500 mt-1">Kelola data infrastruktur sub polder dan lokasinya</p>
        </div>
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.sub-polders.create') }}" class="inline-flex items-center justify-center px-6 py-3 bg-primary-600 text-white rounded-xl font-medium hover:bg-primary-700 focus:ring-4 focus:ring-primary-500/20 transition-all shadow-sm">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/80 border-b border-slate-200 text-sm font-semibold text-slate-600 uppercase tracking-wider">
                        ...
                        <th class="px-6 py-4">Merk / Jenis Pompa</th>
                        <th class="px-6 py-4 text-center">Unit</th>
                        <th class="px-6 py-4 text-center">Kapasitas (m³/s)</th>
                        <th class="px-6 py-4">Status</th>
                        <th class="px-6 py-4 text-right">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 text-sm">
                    @forelse($subPolders as $subPolder)
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-lg bg-slate-100 flex-shrink-0 overflow-hidden border border-slate-200">
                                    @if(is_array($subPolder->photo) && count($subPolder->photo) > 0)
                                        <img src="{{ asset('storage/' . $subPolder->photo[0]) }}" class="w-full h-full object-cover" alt="Sub Polder">
                                    @else
                                ...
                                <div>
                                    <h3 class="font-bold text-slate-900 group-hover:text-primary-600 transition-colors">{{ $subPolder->nama_lokasi ?: '-' }}</h3>
                                    <p class="text-xs text-slate-500 line-clamp-1" title="{{ $subPolder->alamat }}">{{ Str::limit($subPolder->alamat ?: 'Tidak ada alamat', 30) }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="font-medium text-slate-700">{{ $subPolder->merk_pompa ?: '-' }}</div>
                            <div class="text-xs text-slate-500">{{ $subPolder->jenis_pompa ?: '-' }}</div>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-slate-100 text-slate-800">
                                {{ $subPolder->jumlah_unit ?: '0' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold bg-primary-50 text-primary-700">
                                {{ $subPolder->total_kapasitas ?: '0' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            @if($subPolder->status == 1)
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('admin.sub-polders.edit', $subPolder) }}" class="p-2 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                
                                <form action="{{ route('admin.sub-polders.destroy', $subPolder) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data Sub Polder ini?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-base font-medium text-slate-900">Belum ada data Sub Polder</p>
                                <p class="mt-1 text-sm">Klik tombol "Tambah Sub Polder" untuk mulai menambahkan data.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
...
<div id="importModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden">
        <form action="{{ route('admin.sub-polders.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="p-6 border-b border-slate-100">
                <h3 class="text-xl font-bold text-slate-900">Import Data Sub Polder (Excel)</h3>
            </div>
