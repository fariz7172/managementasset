...
@section('title', isset($subPolder) ? 'Edit Sub Polder' : 'Tambah Sub Polder')

@push('styles')
<link rel="stylesheet" href="https://unpkg.com/maplibre-gl@3.6.2/dist/maplibre-gl.css" />
<style>
    /* Prevent leaflet map from overlapping fixed headers */
...
<div class="max-w-5xl mx-auto pb-12">
    ...
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('admin.sub-polders.index') }}" class="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:text-primary-600 hover:border-primary-200 hover:bg-primary-50 transition-all shadow-sm">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
        </a>
        <div>
            <h1 class="text-3xl font-display font-bold text-slate-900">{{ isset($subPolder) ? 'Edit Sub Polder' : 'Tambah Sub Polder Baru' }}</h1>
            <p class="text-slate-500 mt-1">Lengkapi informasi detail mengenai infrastruktur Sub Polder</p>
        </div>
    </div>

    <!-- Form Container -->
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <form action="{{ isset($subPolder) ? route('admin.sub-polders.update', $subPolder) : route('admin.sub-polders.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if(isset($subPolder))
                @method('PUT')
            @endif
            ...
            <div class="p-8 space-y-8">
                <!-- Basic Info Section -->
                <div>
                    <h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider mb-6 pb-2 border-b border-slate-100">Informasi Dasar</h3>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Nama Lokasi -->
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-slate-700">Nama & Lokasi</label>
                            <input type="text" name="nama_lokasi" value="{{ old('nama_lokasi', $subPolder->nama_lokasi ?? '') }}" 
                                ...
                                placeholder="Contoh: Sub Polder Sunter 1B">
                        </div>

                        <!-- Merk Pompa -->
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-slate-700">Merk Pompa</label>
                            <input type="text" name="merk_pompa" value="{{ old('merk_pompa', $subPolder->merk_pompa ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                                placeholder="Contoh: DRAKOS POLIMES">
                        </div>

                        <!-- Jenis Pompa -->
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-slate-700">Jenis Pompa</label>
                            <input type="text" name="jenis_pompa" value="{{ old('jenis_pompa', $subPolder->jenis_pompa ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                                placeholder="Contoh: SUBMERSIBLE">
                        </div>

                        <!-- Jumlah Unit -->
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-slate-700">Jumlah Unit</label>
                            <input type="number" name="jumlah_unit" value="{{ old('jumlah_unit', $subPolder->jumlah_unit ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                        ...
                        <div class="space-y-2 md:col-span-2">
                            <label class="block text-sm font-bold text-slate-700">Total Kapasitas (m³/dtk)</label>
                            <input type="number" step="0.01" name="total_kapasitas" value="{{ old('total_kapasitas', $subPolder->total_kapasitas ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <select name="status" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all">
                                <option value="1" {{ old('status', $subPolder->status ?? '1') == '1' ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ old('status', $subPolder->status ?? '') == '0' ? 'selected' : '' }}>Perbaikan</option>
                            </select>
                        </div>
                        
                        <!-- Deskripsi -->
                        <div class="space-y-2 md:col-span-2">
                            <label class="block text-sm font-bold text-slate-700">Deskripsi / Keterangan</label>
                            <textarea name="deskripsi" rows="3" 
                                ...
                                placeholder="Tambahkan deskripsi atau keterangan...">{{ old('deskripsi', $subPolder->deskripsi ?? '') }}</textarea>
                        </div>
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <textarea name="alamat" rows="2" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all placeholder:text-slate-400"
                                placeholder="Masukkan alamat lengkap lokasi sub polder...">{{ old('alamat', $subPolder->alamat ?? '') }}</textarea>
                        </div>
                        
                        <!-- Map Container -->
                        <div class="space-y-3 md:col-span-2">
                            <label class="block text-sm font-bold text-slate-700">Pilih Lokasi di Peta</label>
                        ...
                        <!-- Longitude -->
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-slate-700">Longitude</label>
                            <input type="text" name="longitude" id="input-longitude" value="{{ old('longitude', $subPolder->longitude ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                        ...
                        <!-- Latitude -->
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-slate-700">Latitude</label>
                            <input type="text" name="latitude" id="input-latitude" value="{{ old('latitude', $subPolder->latitude ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                ...
                <!-- Media Section -->
                <div>
                    <h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider mb-6 pb-2 border-b border-slate-100">Foto & Media</h3>
                    
                    <div class="space-y-4">
                        <label class="block text-sm font-bold text-slate-700">Upload Foto Sub Polder</label>
                        ...
                        <!-- Existing Photos -->
                        @if(isset($subPolder) && is_array($subPolder->photo) && count($subPolder->photo) > 0)
                            <div class="mt-6">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4" id="existing-photos">
                                    @foreach($subPolder->photo as $index => $photo)
                                        <div class="relative group rounded-xl overflow-hidden aspect-video border border-slate-200" id="photo-container-{{ $index }}">
                                            ...
                                            <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                <button type="button" onclick="deleteImage({{ $subPolder->id }}, {{ $index }})" class="p-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors shadow-lg">
            ...
            <div class="p-8 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3">
                <a href="{{ route('admin.sub-polders.index') }}" class="px-6 py-2.5 rounded-xl text-slate-600 font-bold hover:bg-slate-200 transition-colors">Batal</a>
                <button type="submit" class="px-8 py-2.5 bg-primary-600 text-white rounded-xl font-bold shadow-sm shadow-primary-500/30 hover:bg-primary-700 focus:ring-4 focus:ring-primary-500/20 transition-all">
                    {{ isset($subPolder) ? 'Simpan Perubahan' : 'Tambahkan Sub Polder' }}
                </button>
            </div>
        </form>
    </div>
...
<script>
    ...
    function deleteImage(subPolderId, index) {
        if(confirm('Yakin ingin menghapus foto ini?')) {
            fetch(`/admin/sub_polders/image/${subPolderId}/${index}`, {
                method: 'DELETE',
                headers: {
