...
@section('title', isset($pompa) ? 'Edit Pompa' : 'Tambah Pompa')
...
<div class="max-w-5xl mx-auto pb-12">
    ...
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('admin.pompas.index') }}" class="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:text-primary-600 hover:border-primary-200 hover:bg-primary-50 transition-all shadow-sm">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
        </a>
        <div>
            <h1 class="text-3xl font-display font-bold text-slate-900">{{ isset($pompa) ? 'Edit Pompa' : 'Tambah Pompa Baru' }}</h1>
            <p class="text-slate-500 mt-1">Lengkapi informasi detail mengenai data pompa</p>
        </div>
    </div>

    <!-- Form Container -->
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <form action="{{ isset($pompa) ? route('admin.pompas.update', $pompa) : route('admin.pompas.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if(isset($pompa))
                @method('PUT')
            @endif
            ...
            <div class="p-8 space-y-8">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Nama Pompa -->
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-slate-700">Nama Pompa</label>
                            <input type="text" name="nama" value="{{ old('nama', $pompa->nama ?? '') }}" 
                                ...
                                placeholder="Contoh: Pompa Ancol">
                        </div>

                        <!-- Merk Pompa -->
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-slate-700">Merk Pompa</label>
                            <input type="text" name="merk_pompa" value="{{ old('merk_pompa', $pompa->merk_pompa ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                                placeholder="Contoh: Ebara / KSB">
                        </div>

                        <!-- Jenis Pompa -->
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-slate-700">Jenis Pompa</label>
                            <input type="text" name="jenis_pompa" value="{{ old('jenis_pompa', $pompa->jenis_pompa ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                        ...
                        <!-- Merk (Pabrikan) -->
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-slate-700">Merk (Pabrikan)</label>
                            <input type="text" name="merk" value="{{ old('merk', $pompa->merk ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" name="jumlah" value="{{ old('jumlah', $pompa->jumlah ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                        ...
                        <!-- Tahun -->
                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-slate-700">Tahun Pembuatan/Instalasi</label>
                            <input type="text" name="tahun" value="{{ old('tahun', $pompa->tahun ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                ...
                <!-- Location Section -->
                <div>
                    <h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider mb-6 pb-2 border-b border-slate-100">Lokasi & Status</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <textarea name="alamat" rows="2" 
                                ...
                                placeholder="Masukkan alamat lengkap lokasi pompa...">{{ old('alamat', $pompa->alamat ?? '') }}</textarea>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="longitude" id="input-longitude" value="{{ old('longitude', $pompa->longitude ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="latitude" id="input-latitude" value="{{ old('latitude', $pompa->latitude ?? '') }}" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="status" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all">
                                <option value="1" {{ old('status', $pompa->status ?? '1') == '1' ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ old('status', $pompa->status ?? '') == '0' ? 'selected' : '' }}>Perbaikan</option>
                            </select>
                        </div>
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <textarea name="deskripsi" rows="3" 
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all placeholder:text-slate-400"
                                placeholder="Tambahkan deskripsi atau keterangan...">{{ old('deskripsi', $pompa->deskripsi ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        <label class="block text-sm font-bold text-slate-700">Upload Foto Pompa</label>
                        ...
                        <!-- Existing Photos -->
                        @if(isset($pompa) && is_array($pompa->photo) && count($pompa->photo) > 0)
                            <div class="mt-6">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4" id="existing-photos">
                                    @foreach($pompa->photo as $index => $photo)
                                        <div class="relative group rounded-xl overflow-hidden aspect-video border border-slate-200" id="photo-container-{{ $index }}">
                                            ...
                                            <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                <button type="button" onclick="deleteImage({{ $pompa->id }}, {{ $index }})" class="p-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors shadow-lg">
            ...
            <div class="p-8 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3">
                <a href="{{ route('admin.pompas.index') }}" class="px-6 py-2.5 rounded-xl text-slate-600 font-bold hover:bg-slate-200 transition-colors">Batal</a>
                <button type="submit" class="px-8 py-2.5 bg-primary-600 text-white rounded-xl font-bold shadow-sm shadow-primary-500/30 hover:bg-primary-700 focus:ring-4 focus:ring-primary-500/20 transition-all">
                    {{ isset($pompa) ? 'Simpan Perubahan' : 'Tambahkan Pompa' }}
                </button>
            </div>
        </form>
    </div>
...
<script>
    ...
    function deleteImage(pompaId, index) {
        if(confirm('Yakin ingin menghapus foto ini?')) {
            fetch(`/admin/pompas/image/${pompaId}/${index}`, {
                method: 'DELETE',
                headers: {
